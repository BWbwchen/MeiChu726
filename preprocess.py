...
def preprocess (wtf, ttype):
    ...
    for i in range(0, len(input_raw_data)):
        if start == -1 :
            ...
            if state != ttype[i] and state != 'Pressed' and state != 'Released':
    ...
    for index, input_data in enumerate(li) :
        ...
        # elapse time
        elapse_time = input_data[0, -1] - input_data[0, 0]

        # trajectory length
        trajectory_length = s[-1]

        # distance end to end 
        distance_end_to_end = math.sqrt((input_data[1, -1] - input_data[1, 0]) ** 2 +  (input_data[1, -1] - input_data[1, 0]) ** 2)
...
def get_user_data(file_name):
    with open(file_name, "r") as fh:
        ...
        return pd.DataFrame(preprocess(li, ttype))
